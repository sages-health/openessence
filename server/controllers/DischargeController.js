...
var Discharge = require('../models/Discharge');

module.exports = codex.controller(Discharge, {
  get: true,
  search: true,
  insert: true,
  replace: true,
  preInsert: function (req, esRequest, callback) {
    ...
    Discharge.search({
      ...
      if (results.length > 0 && results[0].id !== esRequest.id) {
        return callback(Boom.create(400, 'There\'s already a discharge type with the name ' + esRequest.body.name, {
