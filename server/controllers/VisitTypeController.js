...
var VisitType = require('../models/VisitType');

module.exports = codex.controller(VisitType, {
  get: true,
  search: true,
  insert: true,
  replace: true,
  preInsert: function (req, esRequest, callback) {
    ...
    VisitType.search({
      ...
      if (results.length > 0 && results[0].id !== esRequest.id) {
        return callback(Boom.create(400, 'There\'s already a visit type with the name ' + esRequest.body.name, {
