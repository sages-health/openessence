...
angular.module(controllers.name).controller('SyndromeEditCtrl', function ($scope, $modal, orderByFilter, gettextCatalog,
                                                                          FrableParams, Syndrome, sortString, $rootScope) {
  $scope.filters = [
    {filterId: 'name'}
  ];
  $scope.filterTypes = [
  ...
  var openDialog = function (record) {
    return $modal.open({
      template: require('../../../partials/edit/forms/syndrome-form.html'),
      controller: ['$scope', '$modalInstance', function ($scope, $modalInstance) {
        $scope.record = record || {};
        $scope.syndrome = angular.copy($scope.record._source) || {};
        ...
        $scope.submit = function (form) {
          ...
          var showError = function (response) {
            if (response.status === 409) {
              ...
              Syndrome.get({_id: $scope.record._id}, function (newData) {
                $scope.conflictError = true;
                $scope.record = newData;
                $scope.syndrome = newData._source;
              });
          ...
          if ($scope.record._id || $scope.record._id === 0) { // TODO move this logic to resource
            ...
            }, $scope.syndrome, cleanup, showError);
          } else {
            Syndrome.save($scope.syndrome, cleanup, showError);
          }
        };
    ...
    $modal.open({
      ...
      controller: ['$scope', '$modalInstance', function ($scope, $modalInstance) {
        ...
        $scope.delete = function () {
          Syndrome.remove({_id: record._id}, function () {
